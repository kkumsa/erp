<x-filament-panels::page>
    ...
    <x-slide-over-panel
        storage-key="opportunityPanelWidth"
        :show="$slideOverMode && $selectedRecordId"
        open-event="record-selected"
    >
        <x-slot name="title">
            {{ $selectedRecord?->name ?? '영업 기회 상세' }}
        </x-slot>

        <x-slot name="headerActions">
            @if($selectedRecordId)
                <a
                    href="{{ \App\Filament\Resources\OpportunityResource::getUrl('view', ['record' => $selectedRecordId]) }}"
                ...
                <a
                    href="{{ \App\Filament\Resources\OpportunityResource::getUrl('edit', ['record' => $selectedRecordId]) }}"
