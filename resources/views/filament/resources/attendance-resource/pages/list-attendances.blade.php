<x-filament-panels::page>
    ...
    <x-slide-over-panel
        storage-key="attendancePanelWidth"
        :show="$slideOverMode && $selectedRecordId"
        open-event="record-selected"
    >
        <x-slot name="title">
            {{ $selectedRecord?->employee?->user?->name ?? '근태 상세' }}
        </x-slot>

        <x-slot name="headerActions">
            @if($selectedRecordId)
                <a
                    href="{{ \App\Filament\Resources\AttendanceResource::getUrl('edit', ['record' => $selectedRecordId]) }}"
