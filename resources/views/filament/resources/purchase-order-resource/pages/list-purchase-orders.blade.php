<x-filament-panels::page>
    ...
    <x-slide-over-panel
        storage-key="purchaseOrderPanelWidth"
        :show="$slideOverMode && $selectedRecordId"
        open-event="record-selected"
    >
        <x-slot name="title">
            {{ $selectedRecord?->po_number ?? '구매주문 상세' }}
        </x-slot>

        <x-slot name="headerActions">
            @if($selectedRecordId)
                <a
                    href="{{ \App\Filament\Resources\PurchaseOrderResource::getUrl('view', ['record' => $selectedRecordId]) }}"
                ...
                <a
                    href="{{ \App\Filament\Resources\PurchaseOrderResource::getUrl('edit', ['record' => $selectedRecordId]) }}"
        ...
        @if($selectedRecordId)
            ...
            <div class="mt-6">
                <x-approval-status-panel :record="$selectedRecord" />
            </div>
