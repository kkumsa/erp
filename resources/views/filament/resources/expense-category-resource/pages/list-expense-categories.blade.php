<x-filament-panels::page>
    ...
    <x-slide-over-panel
        storage-key="expenseCategoryPanelWidth"
        :show="$slideOverMode && $selectedRecordId"
        open-event="record-selected"
    >
        <x-slot name="title">
            {{ $selectedRecord?->name ?? '비용 카테고리 상세' }}
        </x-slot>

        <x-slot name="headerActions">
            @if($selectedRecordId)
                <a
                    href="{{ \App\Filament\Resources\ExpenseCategoryResource::getUrl('edit', ['record' => $selectedRecordId]) }}"
