<x-filament-panels::page>
    ...
    <x-slide-over-panel
        storage-key="productCategoryPanelWidth"
        :show="$slideOverMode && $selectedRecordId"
        open-event="record-selected"
    >
        <x-slot name="title">
            {{ $selectedRecord?->name ?? '상품 카테고리 상세' }}
        </x-slot>

        <x-slot name="headerActions">
            @if($selectedRecordId)
                <a
                    href="{{ \App\Filament\Resources\ProductCategoryResource::getUrl('edit', ['record' => $selectedRecordId]) }}"
