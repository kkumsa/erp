<x-filament-panels::page>
    ...
    <x-slide-over-panel
        storage-key="customerPanelWidth"
        :show="$slideOverMode && $selectedRecordId"
        open-event="record-selected"
    >
        <x-slot name="title">
            {{ $selectedRecord?->company_name ?? '고객 상세' }}
        </x-slot>

        <x-slot name="headerActions">
            @if($selectedRecordId)
                <a
                    href="{{ \App\Filament\Resources\CustomerResource::getUrl('view', ['record' => $selectedRecordId]) }}"
                ...
                <a
                    href="{{ \App\Filament\Resources\CustomerResource::getUrl('edit', ['record' => $selectedRecordId]) }}"
        ...
        @if($selectedRecordId)
            ...
            <div class="mt-6">
                @livewire(\App\Filament\Resources\CustomerResource\RelationManagers\ContactsRelationManager::class, [
                    'ownerRecord' => $selectedRecord,
                    'pageClass' => \App\Filament\Resources\CustomerResource\Pages\ViewCustomer::class,
                ], key('contacts-' . $selectedRecordId))
            </div>

            <div class="mt-6">
                @livewire(\App\Filament\Resources\CustomerResource\RelationManagers\OpportunitiesRelationManager::class, [
                    'ownerRecord' => $selectedRecord,
                    'pageClass' => \App\Filament\Resources\CustomerResource\Pages\ViewCustomer::class,
                ], key('opportunities-' . $selectedRecordId))
            </div>
