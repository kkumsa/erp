<x-filament-panels::page>
    ...
    <x-slide-over-panel
        storage-key="invoiceItemPanelWidth"
        :show="$slideOverMode && $selectedRecordId"
        open-event="record-selected"
    >
        <x-slot name="title">
            {{ $selectedRecord?->description ?? '청구서 항목 상세' }}
        </x-slot>

        <x-slot name="headerActions">
            @if($selectedRecordId)
                <a
                    href="{{ \App\Filament\Resources\InvoiceItemResource::getUrl('edit', ['record' => $selectedRecordId]) }}"
