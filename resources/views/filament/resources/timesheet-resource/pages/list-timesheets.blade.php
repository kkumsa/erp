<x-filament-panels::page>
    ...
    <x-slide-over-panel
        storage-key="timesheetPanelWidth"
        :show="$slideOverMode && $selectedRecordId"
        open-event="record-selected"
    >
        <x-slot name="title">
            {{ $selectedRecord?->date?->format('Y-m-d') ?? '타임시트 상세' }}
        </x-slot>

        <x-slot name="headerActions">
            @if($selectedRecordId)
                <a
                    href="{{ \App\Filament\Resources\TimesheetResource::getUrl('edit', ['record' => $selectedRecordId]) }}"
