<x-filament-panels::page>
    ...
    <x-slide-over-panel
        storage-key="purchaseOrderItemPanelWidth"
        :show="$slideOverMode && $selectedRecordId"
        open-event="record-selected"
    >
        <x-slot name="title">
            {{ $selectedRecord?->description ?? '구매주문 항목 상세' }}
        </x-slot>

        <x-slot name="headerActions">
            @if($selectedRecordId)
                <a
                    href="{{ \App\Filament\Resources\PurchaseOrderItemResource::getUrl('edit', ['record' => $selectedRecordId]) }}"
