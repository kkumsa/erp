<x-filament-panels::page>
    ...
    <x-slide-over-panel
        storage-key="contractPanelWidth"
        :show="$slideOverMode && $selectedRecordId"
        open-event="record-selected"
    >
        <x-slot name="title">
            {{ $selectedRecord?->title ?? '계약 상세' }}
        </x-slot>

        <x-slot name="headerActions">
            @if($selectedRecordId)
                <a
                    href="{{ \App\Filament\Resources\ContractResource::getUrl('view', ['record' => $selectedRecordId]) }}"
                ...
                <a
                    href="{{ \App\Filament\Resources\ContractResource::getUrl('edit', ['record' => $selectedRecordId]) }}"
