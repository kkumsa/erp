...
class ListStocks extends ListRecords implements HasInfolists
{
    ...
    public ?Stock $selectedRecord = null;

    public function selectRecord(int $recordId): void
    {
        if ($this->slideOverMode) {
            $this->selectedRecordId = $recordId;
            $this->selectedRecord = Stock::find($recordId);
            $this->dispatch('record-selected');
        } else {
            $this->redirect(StockResource::getUrl('edit', ['record' => $recordId]));
        }
    }
    ...
    public function recordInfolist(Infolist $infolist): Infolist
    {
        ...
        return StockResource::infolist($infolist)
