...
class ListPayments extends ListRecords implements HasInfolists
{
    ...
    public ?Payment $selectedRecord = null;

    public function selectRecord(int $recordId): void
    {
        if ($this->slideOverMode) {
            $this->selectedRecordId = $recordId;
            $this->selectedRecord = Payment::find($recordId);
            $this->dispatch('record-selected');
        } else {
            $this->redirect(PaymentResource::getUrl('edit', ['record' => $recordId]));
        }
    }
