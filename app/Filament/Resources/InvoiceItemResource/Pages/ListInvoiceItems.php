...
class ListInvoiceItems extends ListRecords implements HasInfolists
{
    use InteractsWithInfolists;

    protected static string $resource = InvoiceItemResource::class;

    protected static string $view = 'filament.resources.invoice-item-resource.pages.list-invoice-items';

    public bool $slideOverMode = false;

    public ?int $selectedRecordId = null;

    public ?InvoiceItem $selectedRecord = null;

    public function selectRecord(int $recordId): void
    {
        if ($this->slideOverMode) {
            $this->selectedRecordId = $recordId;
            $this->selectedRecord = InvoiceItem::find($recordId);
            $this->dispatch('record-selected');
        } else {
            $this->redirect(InvoiceItemResource::getUrl('edit', ['record' => $recordId]));
        }
    }
    ...
    public function recordInfolist(Infolist $infolist): Infolist
    {
        ...
        return InvoiceItemResource::infolist($infolist)
