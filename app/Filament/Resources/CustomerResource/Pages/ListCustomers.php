...
class ListCustomers extends ListRecords implements HasInfolists
{
    ...
    public ?Customer $selectedRecord = null;

    public function selectRecord(int $recordId): void
    {
        if ($this->slideOverMode) {
            $this->selectedRecordId = $recordId;
            $this->selectedRecord = Customer::find($recordId);
            $this->dispatch('record-selected');
        } else {
            $this->redirect(CustomerResource::getUrl('view', ['record' => $recordId]));
        }
    }
    ...
    public function recordInfolist(Infolist $infolist): Infolist
    {
        ...
        return CustomerResource::infolist($infolist)
