    ...
    protected function getHeaderActions(): array
    {
        return [
            Actions\Action::make('submitApproval')
                ...
                ->form(function () {
                    $flows = ApprovalFlow::where('target_type', 'App\\Models\\Leave')
                    ...
                    $days = $this->record->days . '일';
                    return [
                        Forms\Components\Placeholder::make('info')
                            ->label('휴가 정보')
                            ->content($this->record->start_date?->format('Y-m-d') . ' ~ ' . $this->record->end_date?->format('Y-m-d') . " ({$days})"),
