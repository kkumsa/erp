...
class ListTimesheets extends ListRecords implements HasInfolists
{
    ...
    public ?Timesheet $selectedRecord = null;

    public function selectRecord(int $recordId): void
    {
        if ($this->slideOverMode) {
            $this->selectedRecordId = $recordId;
            $this->selectedRecord = Timesheet::find($recordId);
            $this->dispatch('record-selected');
        } else {
            $this->redirect(TimesheetResource::getUrl('edit', ['record' => $recordId]));
        }
    }
    ...
    public function recordInfolist(Infolist $infolist): Infolist
    {
        ...
        return TimesheetResource::infolist($infolist)
